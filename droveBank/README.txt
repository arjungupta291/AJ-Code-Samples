...
3.) The third module is called PiggyNetwork.py. This contains the framework for our two servers to run concurrently using threads. When
initialized, it simply creates two server instances with the appropriate attributes and adds each server to the other's network. The
only notable method here is the initialize() method which starts up our concurrent system.
